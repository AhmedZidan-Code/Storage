        ...
        <div class="d-flex flex-column mb-7 fv-row col-sm-6">
            ...
            <select id="branch_id" name="branch_id" class="form-control">
                ...
                @foreach ($branches as $branch)
                    <option value="{{ $branch->id }}" {{ $representative->branch_id == $branch->id ? 'selected' : '' }}>
                         {{ $branch->title }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="d-flex flex-column mb-7 fv-row col-sm-6">
            ...
            <select id="storage_id" name="storage_id" class="form-control">
                ...
                @foreach ($storages as $storage)
                    <option value="{{ $storage->id }}" {{ $representative->storage_id == $storage->id ? 'selected' : '' }}> {{ $storage->title }}</option>
                @endforeach
            </select>
