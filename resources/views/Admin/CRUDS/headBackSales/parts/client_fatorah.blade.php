                        ...
                        <tr id="tr-{{ $key }}">
                            ...
                            <th style="width: 200px;">
                                <div class="d-flex flex-column mb-7 fv-row col-sm-2 " >
                                    ...
                                    <select class="form-control changeKhamId" data-id="{{ $key }}" name="productive_id[]"
                                        id='productive_id-{{ $key }}' readonly style="width: 200px;"  style="pointer-events: none;">
                            ...
                            <th>
                                <input data-id="{{ $key }}" step=".1" onchange="callTotal()"
                                    ...
                                    id="productive_sale_price-{{ $key }}" style="width: 100%;" disabled>

                            </th>
                            <th style="padding: 8px;">
                                <input data-id="{{ $key }}" step=".1" type="number"
                                    value="{{ $pivot->bouns }}" min="0" name="bouns[]"
                                    id="bouns-{{ $key }}" style="width: 100px; text-align: center;" disabled>
                            </th>
                            <th style="padding: 8px;">
                                <input data-id="{{ $key }}" step=".1" type="number"
                                    value="{{ $pivot->discount_percentage }}" min="0"
                                    name="discount_percentage[]" id="discount_percentage-{{ $key }}"
                                    style="width: 100px; text-align: center;" onkeyup="callTotal()" disabled>
                            </th>
                            <th style="padding: 8px;">
                                <input data-id="{{ $key }}" step=".1" type="number"
                                    value="{{ $pivot->batch_number }}" min="0" name="batch_number[]"
                                    id="batch_number-{{ $key }}" style="width: 100px; text-align: center;" disabled>
                            </th>
                            <th>
                                <input type="number" disabled value="{{ $pivot->total }}" min="1"
                                    name="total[]" id="total-{{ $key }}" style="width: 100%;" disabled>
                    ...
                    <tr>
                        ...
                        <th colspan="2" style="text-align: center; background-color: gray">
                            <input type="number" id="total_discount" value="{{ $row->total_discount }}" min="0" max="99"
                                name="total_discount" style="width: 100%;" onkeyup="totalAfterDiscount()" disabled> <!-- Adjusted width -->
                        </th>
